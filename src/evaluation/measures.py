...
def compute_area_under_curve(roc_values, x_measure, y_measure):
    ...
    if x_limit < 1:
        ...
        intermediate_y_val = ((x_limit - prev_x_val)/(next_x_val - prev_x_val)) * (next_y_val - prev_y_val) + prev_y_val
...
def example():
    ...
    AUC = compute_area_under_curve(roc_values, x_label="FAR", y_label="TPR")
    print("AUC: " + str(AUC))
    AUC5 = compute_area_under_curve(roc_values, x_label="FAR*0.05", y_label="TPR")
    print("Partial AUC with FAR<=5%: " + str(AUC5))
    
    
    dAUC = compute_area_under_curve(roc_values, x_label="FAR", y_label="detectionRate")
    print("dAUC: " + str(dAUC))
    dAUC5 = compute_area_under_curve(roc_values, x_label="FAR*0.05", y_label="detectionRate")
    print("Partial dAUC with FAR<=5%: " + str(dAUC5))
    
    
    
    amoc_auc = compute_area_under_curve(roc_values, x_label="FAR", y_label="detectionDelay")
    print("AMOC AUC: " + str(amoc_auc))
    amoc_auc5 = compute_area_under_curve(roc_values, x_label="FAR*0.05", y_label="detectionDelay")
