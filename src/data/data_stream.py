...
class WSARE_DATA:
    ...
    ***********************************************************************************************************
    ...
    def _load_data(self):
        ...
        if not self.environmental_variables:
            self.environmental_df = self.environmental_df[["time_slot"]]
    ...
    ***********************************************************************************************************
    ...
    def _load_data(self):
        ...
        if io.exist(self.get_ident(), loc=cache_location):
            ...
            if self.data_stream_id == -1:  # add no outbreak
                self.syndrome = None
                self.outbreak_size = 0
                self.outbreaks = []
...
# two year with extra env "weather" and "flu"; "week_day" only 3 values
class ED_DATA_RANDOM_ONE_STREAM3:

    def __init__(self, outbreak_sizes=40, n_syndromes=100, environmental_variables=True, random_seed=0):
        """
        Initializing
        """
        self.outbreak_sizes = outbreak_sizes
    ...
    ***********************************************************************************************************
    ...
    def _get_syndromes(self):
        ...
        # Replace normal values with nan, these will not be considered for defining outbreaks
        copy_cases_df["mts"] = copy_cases_df["mts"].replace(["Sonstiges"], np.nan)
        copy_cases_df["mts"] = copy_cases_df["mts"].replace(["Unwohlsein"], np.nan)
        copy_cases_df["fever"] = copy_cases_df["fever"].replace(["normal"], np.nan)
        copy_cases_df["pulse"] = copy_cases_df["pulse"].replace(["normal"], np.nan)
        copy_cases_df["respiration"] = copy_cases_df["respiration"].replace(["10-20 (normal)"], np.nan)
        copy_cases_df["oxygensaturation"] = copy_cases_df["oxygensaturation"].replace([">=95 (normal)"], np.nan)
    ...
    def _select_syndrome(self, cur_index, syndromes, rand_gen):
        # select syndrome
        if cur_index < 20:
            syndromes = list(filter(lambda x: x[1] <= 1, syndromes))
            syndrome = syndromes[rand_gen.randint(len(syndromes))]
        else:
            syndromes = list(filter(lambda x: x[1] > 1, syndromes))
            syndrome = syndromes[rand_gen.randint(len(syndromes))]
    ...
    def _load_data(self):
        ...
        if io.exist(self.get_ident(), loc=cache_location):
            [self.cases_df, self.environmental_df, self.history, self.start_test_part, self.last_time_slot,
             self.outbreaks, self.selected_syndromes, self.outbreak_sizes] = io.load(self.get_ident(), loc=cache_location)
        else:

            # load basic data
            load_path = io.get_project_directory() + "_data/ed_data/ed1_v5.csv"
            df = pd.read_csv(load_path)

            # select 2016 and 2017
            df = df[df["time_slot"] >= 365]
            df = df[df["time_slot"] <= 1095]
            df["time_slot"] -= 365
            ...
            self.cases_df = df[["time_slot", "gender", "age", "mts", "fever", "pulse",
                                "respiration", "oxygensaturation", "bloodpressure"]]
            environmentals = df[["time_slot", "week_day", "season", "flu", "weather"]]
            self.environmental_df = environmentals.drop_duplicates()

            syndromes = self._get_syndromes()
            self.outbreaks = []
            self.selected_syndromes = []
            for i in range(self.n_syndromes):

                # initialize random number generator
                rand_gen = np.random.RandomState(seed=self.random_seed + i * 10000)
                syndrome = self._select_syndrome(i, syndromes, rand_gen)
                self.selected_syndromes.append(syndrome)

                outbreak_time_slot = rand_gen.randint(low=self.start_test_part, high=self.last_time_slot + 1)

                # generate outbreak cases
                sample_df = self.cases_df
                for [cond_col, cond_val] in syndrome:
                    sample_df = sample_df[sample_df[cond_col] == cond_val]
                sample_df = sample_df.sample(self.outbreak_sizes, random_state=rand_gen, replace=True)

                for outbreak_size in range(1, self.outbreak_sizes + 1):
                    # set evaluation details and generate random outbreak
                    new_time_slot = self.last_time_slot + i * self.outbreak_sizes + outbreak_size

                    # get cases and env
                    outbreak_day_cases = self.cases_df[self.cases_df["time_slot"] == outbreak_time_slot].copy()
                    outbreak_day_env = self.environmental_df[self.environmental_df["time_slot"] == outbreak_time_slot].copy()
                    cur_sample = sample_df[:outbreak_size].copy()

                    # append particular outbreak day
                    outbreak_day_cases = pd.concat([outbreak_day_cases, cur_sample])
                    outbreak_day_cases["time_slot"] = [new_time_slot] * len(outbreak_day_cases)
                    outbreak_day_env["time_slot"] = [new_time_slot]
                    self.outbreaks.append([outbreak_time_slot, outbreak_time_slot])

                    # add cases to cases dataframe
                    self.cases_df = pd.concat([self.cases_df, outbreak_day_cases])
                    self.environmental_df = pd.concat([self.environmental_df, outbreak_day_env])

            self.cases_df = self.cases_df.sort_values(["time_slot"])
            self.environmental_df = self.environmental_df.sort_values(["time_slot"])
            self.last_time_slot = np.max(self.cases_df["time_slot"])

            # prepare history
            self.history = []
            for time_slot in range(np.min(df["time_slot"]), np.max(self.cases_df["time_slot"]) + 1):
                ts_cases_df = self.cases_df[self.cases_df["time_slot"] == time_slot]
                ts_cases_df = ts_cases_df.drop(['time_slot'], axis=1)
                ts_environmental_df = self.environmental_df[self.environmental_df["time_slot"] == time_slot]
                ts_environmental_df = ts_environmental_df.drop(['time_slot'], axis=1)
            ...
            if not os.path.isdir(io.get_project_directory() + cache_location):
                os.makedirs(io.get_project_directory() + cache_location)
            data = [self.cases_df, self.environmental_df, self.history, self.start_test_part, self.last_time_slot,
                    self.outbreaks, self.selected_syndromes, self.outbreak_sizes]
            io.save(data, self.get_ident(), loc=cache_location)
    ...
    ***********************************************************************************************************
    ...
    def get_info(self):
        ...
        return {
            ...
            "start_test_part": self.start_test_part,
            "outbreaks": self.outbreaks,
            "selected_syndromes": self.selected_syndromes,
            "outbreak_sizes": self.outbreak_sizes
        }
    ...
    ***********************************************************************************************************
    ...
    def get_params(self):
        return [["outbreak_sizes", self.outbreak_sizes], ["n_syndromes", self.n_syndromes],
                ["environmental_variables", self.environmental_variables], ["random_seed", self.random_seed]]

    def get_ident(self):
        return "ED_DATA_RANDOM_ONE_STREAM3$" + "$".join([key + "=" + str(val) for key, val in self.get_params()])
