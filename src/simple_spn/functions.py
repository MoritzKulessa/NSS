...
from simple_spn.internal.InferenceRange import piecewise_likelihood_range, categorical_likelihood_range, \
    identity_likelihood_range, gaussian_likelihood_range
...
def probs(spn, ranges):
    ...
    return Inference.likelihood(spn, ranges, dtype=np.float64, node_likelihood=inference_support_ranges).reshape(
        len(ranges))
...
def log_probs_spflow(spn, data):
    return Inference.log_likelihood(spn, data, dtype=np.float64).reshape(
        len(data))  # Inference.likelihood(spn, data, dtype=np.float64).reshape(len(data))
...
from simple_spn.internal.Expectation import Expectation, piecewise_expectation, identity_expectation, \
    gaussian_expectation, categorical_expectation
...
def expects(spn, feature_id, ranges):
    return Expectation(spn, feature_id, ranges, node_expectation=node_expectation_support,
...
def classify(spn, target_id, rang=None, f_info=None):
    if rang is None: rang = [None] * (np.max(spn.scope) + 1)
    return classifies(spn, target_id, np.array([rang]), f_info)[0]


def classifies(spn, target_id, ranges, f_info=None):
    if f_info is None: f_info = generate_adhoc_feature_info(spn)
    if ranges is None: ranges = np.array([[None] * (np.max(spn.scope) + 1)])
    assert (not any(ranges[:, target_id]))
    ps = []
    for v in range(f_info.get_num_values(target_id)):
        ranges[:, target_id] = NominalRange([v])
        ps.append(probs(spn, ranges))
    return np.argmax(ps, axis=0)


def classify_dataset(spn, target_id, df, transform=False, f_info=None, epsilon=0.01):
    if f_info is None: f_info = generate_adhoc_feature_info(spn)
    sorted_scope = sorted(spn.scope)

    if transform:
        for col_name in df.columns:
            if f_info.is_discrete(col_name):
                df[col_name] = df[col_name].map(f_info.get_inverse_value_mapping(col_name))
    ...
    for i, col in enumerate(values.T):
        ...
        if f_info.is_dicrete(f_id):
            for j, v in enumerate(col): ranges[j, f_id] = NominalRange([v])
        elif f_info.is_numeric(f_id):
            bound = epsilon * (f_info.get_max_value(f_id) - f_info.get_min_value(f_id))
            for j, v in enumerate(col): ranges[j, f_id] = NumericRange([[v - bound, v + bound]])
        else:
            raise Exception("Unknown attribute-type: " + str(f_info.get_feature_type(f_id)))

    return classifies(spn, target_id, ranges, f_info)
...
def marg_rang(spn, rang=None, node_likelihood=inference_support_ranges, keep=None):
...
def sampling(spn, n_samples, random_seed=123):
    ...
    return SamplingRange.sample_instances(spn, len(spn.scope), n_samples, np.random.RandomState(random_seed),
                                          ranges=rang, node_sample=node_sample_support,
...
def sampling_spnflow(spn, evidence, n_samples, random_seed=123):
    data = np.array(list(evidence) * n_samples).reshape(-1, len(evidence))
    return Sampling.sample_instances(spn, data, RandomState(random_seed))
...
def get_overall_population(spn, f_info=None, numeric_prec=50):
    if f_info is None: f_info = generate_adhoc_feature_info(spn)
    ...
    for i, f_id in enumerate(sorted(spn.scope)):
        ...
        result_dict[f_id] = {}
        result_dict[f_id]["feature_name"] = f_info.get_feature_name(f_id)

        if f_info.is_discrete(f_id):
            result_dict[f_id]["feature_type"] = "discrete"

            val_pairs = sorted(f_info.get_value_mapping(f_id).items(), key=lambda x: x[0])
            ...
            x_vals = np.linspace(f_info.get_min_value(f_id), f_info.get_max_value(f_id), numeric_prec, endpoint=True)
            ...
            raise Exception("Unknown attribute-type: " + str(f_info.get_feature_type))
...
def _compute_weighted_mean_and_variance(weights, vals):
    ...
    variance = sum([weights[i] * (val - mean) * (val - mean) for i, val in enumerate(vals)]) / (
                ((non_zero_weights - 1.) * tot_weight) / non_zero_weights)
...
from simple_spn.feature_info import FeatureInfo
...
def transform_dataset(df, feature_types=None):
    ...
    for i, col_name in enumerate(df.columns):
        ...
        if feature_types[i] == "discrete":
            if df[col_name].dtype == np.dtype(np.int64).type:
                ...
                unq_vals = sorted(unq_vals, key=str)
    ...
    return trans_df, FeatureInfo(value_dict), get_standard_parametric_types(feature_types)
    ...
    for col_name in df.columns:
        if df[col_name].dtype == np.dtype(np.float64).type:
            ...
            if n_unique_vals < 30:
                ...
                warnings.warn(
                    "High number of discrete values " + str(n_unique_vals) + "for dataset column: " + col_name)
                feature_types.append("discrete")
            else:
                feature_types.append("numeric")
...
def get_standard_parametric_types(feature_types):
    ...
    for feature_type in feature_types:
        if feature_type == "discrete":
            ...
            raise Exception("Unknown attribute-type: " + str(feature_type))
    return parametric_types


def generate_adhoc_feature_info_from_data(data, parametric_types):
    val_dict = {}
    ...
    return FeatureInfo(val_dict)


def generate_adhoc_feature_info(spn):
    val_dict = {}
...
def plot_spn(spn, fname="plot.pdf"):
    ...
    nx.draw(g, pos, with_labels=True, arrows=False, node_color="#DDDDDD", edge_color="#888888", node_size=1250,
...
def __get_networkx_obj(spn):
    ...
    for n in all_nodes:
        ...
        if isinstance(n, Sum):
            ...
            label = "G" + str(n.scope[0])  # + " (" + str(round(n.mean, 2)) + ", " + str(round(n.stdev, 2)) + ")"
