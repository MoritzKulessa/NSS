...
def marg_rang(spn, rang, node_likelihood, keep=None):
    if rang is None: rang = np.full(len(spn.scope), fill_value=None)

    if keep is None:
        keep = set([i for i, val in enumerate(rang) if val is None])
    else:
        rang_keep = set([i for i, val in enumerate(rang) if val is None])
        keep = keep.intersection(rang_keep)
    ...
    def spn_for_evidence_recursive(node):
        ...
        if len(newNode.scope) == 0: return 1, None
        ...
        if isinstance(node, Sum):
            ...
            newNode.weights = list(new_weights / np.sum(new_weights))
            newNode.children = new_childs

            if len(new_childs) == 0:
                return np.sum(new_weights), None
            else:
