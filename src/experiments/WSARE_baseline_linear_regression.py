...
from algo.baselines import LinearRegression 
...
if __name__ == '__main__':
    ...
    for data_stream_id in data_stream_ids:
        ...
        #configure algorithm
        algo = LinearRegression(data_stream)
        ...
        micro_outbreaks += [[start+len(micro_scores), end+len(micro_scores)] for start, end in outbreaks]
        micro_scores += list(scores)
    
    print()
    print("Macro-averaged area under partial AMOC-curve (FAR <= 5%): " + str(np.mean(measure_results)))
    
    roc_values = measures.compute_roc_values(micro_scores, micro_outbreaks)
    micro_amoc_auc5 = measures.compute_area_under_curve(roc_values, x_measure="FAR*0.05", y_measure="detectionDelay")
    
    print()
    print("Micro-averaged area under partial AMOC-curve (FAR <= 5%): " + str(micro_amoc_auc5))
