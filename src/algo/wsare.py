...
class WSARE_NON_INCREMENTAL(object):

    def __init__(self, syndrome_counter, randomization=10):
        """
        Initialize
        :param syndrome_counter: the module producing syndrome counts
        :param randomization: the number of iterations for the randomization test, if randomization is None then the
    ...
    ***********************************************************************************************************
    ...
    def _obtain_reference_set_WSARE3(self, time_slot):
        ...
        :return: a dataframe containing cases
        """
        tmp_path = io.get_project_directory() + "_tmp/WSARE3_" + self.data_stream.get_ident() + "_" + str(self.randomization) + ".csv"
        tmp_path2 = io.get_project_directory() + "_tmp/WSARE3_" + self.data_stream.get_ident() + "_" + str(self.randomization) + "_samples" + ".csv"
        ...
        # generate the samples
        gum.generateCSV(bn, tmp_path2, 10000, False, with_labels=True)

        # read samples
        samples = pd.read_csv(tmp_path2)
