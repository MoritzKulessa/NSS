...
class DMSS(object):
    ...
    def __init__(self, data_stream, min_support_set=5, min_support_rule=5, ref_windows=[-1]):
        ...
        :param ref_window: reference time slots for creating the reference set. They are relative to the current evaluated time slot. (e.g. [-1, -2] = take the cases of the previous two time slots for the reference set)
        '''
    ...
    ***********************************************************************************************************
    ...
    def evaluate(self, time_slot):
        ...
        for _, row in rules.iterrows():
            ...
            _, p_value = stats.fisher_exact(contigency_table, alternative="two-sided")
